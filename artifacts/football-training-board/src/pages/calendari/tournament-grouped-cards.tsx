...
function isTournamentFutureFixtureSide(value: string): boolean {
  const n = normalizeSide(value);
  if (!n) return true;
  if (isPlaceholderTournamentTeam(value)) return true;
  if (/\bvincente\s+gara\b/.test(n)) return true;
  if (/\b\d{1,2}[\/.\-]\d{1,2}(?:[\/.\-]\d{2,4})?\b/.test(n)) return true;
  if (/\b\d{1,2}[:.]\d{2}\b/.test(n)) return true;
  if (/\b\d+\s*(?:classificat|triangolare|quadrangolare|girone|gold|argento|platino|bronzo)\b/.test(n)) return true;
  return parseTournamentPlacementRef(value) != null;
}

function isTournamentStoredFutureFixtureEntry(entry: TournamentProgramEntry): boolean {
  const id = String(entry.id ?? "");
  return id.startsWith("fixture|");
}

function isRealTournamentMatchEntry(entry: TournamentProgramEntry): boolean {
  if (entry.kind === "composition" || entry.kind === "fixture_placeholder") return false;
...
function standingsFor(
  ...
  return [...table.values()]
    .filter((row) => !isTournamentFutureFixtureSide(row.team))
    .sort(
    (a, b) =>
      b.pts - a.pts ||
      (b.gf - b.gs) - (a.gf - a.gs) ||
...
const FUTURE_COMPOSITION_PHASE_ORDER: Record<string, number> = {
  ...
  [normalizeSide("Argento")]: 2,
  [normalizeSide("Gold")]: 3,
  [normalizeSide("Sesti di finale")]: 4,
  [normalizeSide("Triangolari di semifinale")]: 5,
  [normalizeSide("Finali")]: 6,
};

function futureCompositionPhaseOrder(phase: string | null | undefined): number {
  const key = normalizeSide(String(phase ?? "").trim() || "Fase futura");
  return FUTURE_COMPOSITION_PHASE_ORDER[key] ?? 99;
}

function isFuturePhaseCompositionGroupLabel(label: string): boolean {
  const n = normalizeSide(label);
...
function futureCompositionGroupRank(label: string): number {
  const n = normalizeSide(label);
  const mv = n.match(/^girone\s+([m-v])$/);
  if (mv) return 10 + (mv[1]!.charCodeAt(0) - 96);
  const argento = n.match(/^girone\s+argento\s+([a-z])$/);
  if (argento) return 30 + (argento[1]!.charCodeAt(0) - 96);
  const gold = n.match(/^girone\s+gold\s+([a-z])$/);
  if (gold) return 40 + (gold[1]!.charCodeAt(0) - 96);
  const tri = n.match(/^triangolare\s+(\d+)$/);
  if (tri) return 50 + Number(tri[1]);
  const quad = n.match(/^quadrangolare\s+(\d+)$/);
  if (quad) return 60 + Number(quad[1]);
  const letter = n.match(/\b(?:girone|raggruppamento|triangolare|quadrangolare)\s+([a-z])\b/)?.[1];
  if (letter) return 900 + letter.charCodeAt(0) - 96;
  const number = Number(n.match(/\b(?:girone|raggruppamento|triangolare|quadrangolare)\s+(\d+)\b/)?.[1]);
  if (Number.isFinite(number)) return 900 + 100 + number;
  return 999;
}

function groupFutureCompositionSlotsByPhase(program: TournamentProgramEntry[]): FutureCompositionPhaseView[] {
  const compositionEntries = program.filter((entry) => entry.kind === "composition");
  if (compositionEntries.length === 0) return [];

  const phaseMap = new Map<string, Map<string, Set<string>>>();
  for (const entry of compositionEntries) {
    const groupLabel = programGroupLabel(entry);
    ...
    const groupMap = phaseMap.get(phaseLabel)!;
    if (!groupMap.has(groupLabel)) groupMap.set(groupLabel, new Set());
    groupMap.get(groupLabel)!.add(slot);
  }

  const phases: FutureCompositionPhaseView[] = [];
  for (const [phase, groupMap] of phaseMap.entries()) {
    const groups = [...groupMap.entries()]
      ...
      .sort(
        (a, b) =>
          futureCompositionGroupRank(a.label) - futureCompositionGroupRank(b.label) ||
...
function groupFutureLayoutByPhase(program: TournamentProgramEntry[]): FutureCompositionPhaseView[] {
  ...
  for (const entry of program) {
    if (entry.kind !== "fixture_placeholder" && !isTournamentStoredFutureFixtureEntry(entry)) continue;
...
function tournamentSuppressesAutoGeneratedFinals(
  program: TournamentProgramEntry[],
  compositionPhases: FutureCompositionPhaseView[],
): boolean {
  const compositionGroupLabels = compositionPhases.flatMap((phase) => phase.groups.map((group) => group.label));
  for (const label of compositionGroupLabels) {
    if (isFuturePhaseCompositionGroupLabel(label)) return true;
  }

  const matchGroupLabels = new Set(
    program
      .filter((entry) => isRealTournamentMatchEntry(entry) && entry.group?.trim())
      .map((entry) => normalizeSide(programGroupLabel(entry))),
  );
  if (matchGroupLabels.size > 2 && compositionGroupLabels.length > 0) return true;
  return false;
}
    ...
    <div className="space-y-4 min-w-0">
      {groups.map((g) => {
        ...
        if (finalsRule !== "manual" && generatedFinals.length > 0) {
          let fallbackIndex = 0;
          program
            .filter((entry) => isRealTournamentMatchEntry(entry) && isProgramEntryFinal(entry))
                            ...
                            {group.entries.map((entry) => {
                              ...
                              const displayLabel = generatedFinal
                                ...
                                : (isPlaceholderTournamentTeam(entry.homeTeam) || isPlaceholderTournamentTeam(entry.awayTeam)
                                  ...
                                  : entryLabel);
                              return (
                              <div key={entry.id} className={`py-2 text-xs flex items-center justify-between gap-2 ${nonScoreEntry ? "text-muted-foreground" : ""}`}>
                                <div className="min-w-0">
                                ...
                                {canEditTournamentScores && !nonScoreEntry ? (
                                  <Button
                                    type="button"
                                    variant="ghost"
                                    size="icon"
                  ...
                  <div className="rounded-lg border border-border/80 bg-muted/10 p-3 space-y-2">
                    ...
                    <div className="border-t pt-2">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {suppressAutoGeneratedFinals ? null : (
                          <Button
                            type="button"
                            variant="outline"
                            size="sm"
                      ...
                      {suppressAutoGeneratedFinals ? (
                        <p className="mt-1 text-xs text-muted-foreground">
                          Fasi finali gestite dal programma torneo importato
                        </p>
                      ...
                      {suppressAutoGeneratedFinals ? null : finalsRule === "manual" ? null : generatedFinals.length === 0 ? (
